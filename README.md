...
<p align="center">
  <img alt="version" src="https://img.shields.io/badge/version-2.16.1-2563eb">
...
- **准入与排序分离（v2.16.0）**：两路分数加权混合（α/β 可调）只用于排序；是否注入按通道独立判定——关键词归一 BM25 分 ≥ `min_score` **或** 余弦 ≥ 语义门槛才注入，**宁缺毋滥、不强行凑 top_n**。`min_score`（默认 0.5）随库规模缩放；语义门槛模型无关（以本次查询余弦分布为参照，头部相对中位数有显著分离才启用，`min_gap` 默认 0.25 可调）——无关提问零注入、相关提问精确命中
