...
| 菜单页 | 功能 |
|--------|------|
| **管理** | 项目→条目两级树（类型/草稿/归档徽标 + 标题过滤）、markdown 详情、条目增删改；草稿一键「批准」转正，✨ 优化（需配 LLM） |
| **引导** | 各 agent 检测/安装/卸载、技能状态（等价于命令行 `ok setup`）；reasonix 强制检查三档 |
| **设置** | 全局开关、embedding、LLM、Hook 超时、注入冷却、经验沉淀、泛化门控、规则配置 |
...
| `ok gui` | 打开配置中心（双击 exe 无参数运行同效；OkManager.exe 同入口） |
