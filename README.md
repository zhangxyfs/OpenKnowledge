...
<p align="center">
  <img alt="version" src="https://img.shields.io/badge/version-2.18.0-2563eb">
...
| **日志** | 实时日志查看器：ok（hook/检索本体）、守护、embedding 三来源尾部日志，2 秒轮询；来源 chips 多选 +「仅语义」开关 + 文本过滤输入框，上滚暂停自动贴底 |
...
- **准入与排序分离（v2.16.0）**：两路准入独立判定，融合默认按名次（RRF），模型无关无需调参；`fusion = "weighted"` 可回滚旧 α/β 加权——关键词归一 BM25 分 ≥ `min_score` **或** 余弦 ≥ 语义门槛才注入，**宁缺毋滥、不强行凑 top_n**。`min_score`（默认 0.5）随库规模缩放；语义门槛模型无关（以本次查询余弦分布为参照，头部相对中位数有显著分离才启用，`min_gap` 默认 0.25 可调）——无关提问零注入、相关提问精确命中
