	...
	if err == nil {
		...
		for _, ref := range entryPathRef.FindAllString(body, -1) {
			...
			for _, root := range roots {
				...
				seen[file] = true
				snip := excerptLines(string(data), lines)
...
// excerptLines 按 "30" 或 "30-50" 行号取窗口（前后各放宽 5 行）；无行号取文件头 80 行。
func excerptLines(content, lines string) string {
...
func (h *Handler) apiEntryOptimize(w http.ResponseWriter, r *http.Request) {
	...
	raw, reasoning, err := llmx.New(*prof, timeout).Chat(ctx, optimizeSystemPrompt, user, 4096)
	if err != nil {
	...
	// 无需优化判定：模型自报 no_change，或逐字段与原文相同（模型没听指令时的兜底）。
	// tags 逐值去空白后比较，容忍分隔符与中英文逗号差异。
	if !out.NoChange {
		...
		out.NoChange = strings.TrimSpace(out.Title) == strings.TrimSpace(req.Title) &&
			strings.TrimSpace(out.Summary) == strings.TrimSpace(req.Summary) &&
			strings.TrimSpace(out.Body) == strings.TrimSpace(req.Body) &&
			strings.Join(inTags, ",") == strings.Join(outTags, ",")
	}
	if out.NoChange {
		logOptimize("optimize 结果: no_change（无需优化）")
		writeJSON(w, http.StatusOK, map[string]any{"no_change": true})
		return
	}
