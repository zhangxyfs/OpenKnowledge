...
func TestLoadMissingReturnsDefault(t *testing.T) {
	...
	if cfg.Inject.MandatoryMaxTokens != 2000 {
		t.Fatalf("default inject.mandatory_max_tokens = %v, want 2000", cfg.Inject.MandatoryMaxTokens)
	}
...
func TestFeedbackConfigDefaultAndOverride(t *testing.T) {
	...
	global := filepath.Join(dir, "global.toml")
	project := filepath.Join(dir, "project.toml")
	// 全缺省：enabled 默认 false（read 派发未接通，降权暂不生效），其余默认不变
	cfg, err := LoadMerged(project, global)
	if err != nil {
		t.Fatal(err)
	}
