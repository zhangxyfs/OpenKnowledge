...
func runOK(t *testing.T, home, cwd, stdin string, args ...string) (string, string, int) {
	...
	cmd.Dir = cwd
	cmd.Env = append(os.Environ(), "OK_HOME="+home, "KIMI_CODE_HOME="+filepath.Join(home, "kimi"), "PI_CODING_AGENT_DIR="+filepath.Join(home, "pi"), "OK_ZCODE_HOME="+filepath.Join(home, "zcode-nonexistent"), "OK_REASONIX_HOME="+filepath.Join(home, "reasonix-nonexistent"), "OPENAI_API_KEY=") // 清空 key 保证测试离线；reasonix 指向不存在路径防污染真实用户配置
